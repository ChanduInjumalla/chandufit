...
import { DIET_PLANS, BUDGET_LABELS, getPlanForDate, type BudgetType, type DayPlan, type MealItem, type UserMetrics } from '@/data/dietPlans';
...
const generateDates = () => {
    ...
    for (let i = -14; i <= 7; i++) { const d = new Date(today); d.setDate(today.getDate() + i); dates.push(d); }
    return dates;
};
...
export default function DietPage() {
    ...
    const [nutritionOverrides, setNutritionOverrides] = useState<Record<string, { protein: number; cal: number }>>({});

    // Load saved nutrition overrides from localStorage
    useEffect(() => {
        try {
            const saved = localStorage.getItem('chandufit_nutrition_overrides');
            if (saved) setNutritionOverrides(JSON.parse(saved));
        } catch { }
    }, []);

    const saveOverride = (itemName: string, protein: number, cal: number) => {
        const updated = { ...nutritionOverrides, [itemName]: { protein, cal } };
        setNutritionOverrides(updated);
        localStorage.setItem('chandufit_nutrition_overrides', JSON.stringify(updated));
        setEditingItem(null);
        toast.success(`Updated ${itemName} nutrition`);
    };

    const resetOverride = (itemName: string) => {
        const updated = { ...nutritionOverrides };
        delete updated[itemName];
        setNutritionOverrides(updated);
        localStorage.setItem('chandufit_nutrition_overrides', JSON.stringify(updated));
        toast.success(`Reset ${itemName} to default`);
    };

    // Apply overrides to a meal item
    const getItemNutrition = (item: MealItem) => {
        const override = nutritionOverrides[item.name];
        if (override) {
            // Parse qty to get grams, then recalculate
            const grams = parseFloat(item.qty) || 100;
            return {
                cal: Math.round(override.cal * grams / 100),
                protein: Math.round(override.protein * grams / 100),
            };
        }
        return { cal: item.cal, protein: item.protein };
    };
    const calendarRef = useRef<HTMLDivElement>(null);
    const dates = generateDates();

    // Build user metrics for dynamic scaling
    const userMetrics: UserMetrics | null = metrics ? {
        targetCalories: metrics.targetCalories,
        proteinTarget: metrics.macros?.protein || 0,
        weight: metrics.weight || 75,
        goal: metrics.goal || 'fat_loss',
    } : null;

    // Get plan scaled to user's body — portions auto-adjust
    const mealPlan = getPlanForDate(budget, selectedDate, userMetrics);
    const budgetInfo = BUDGET_LABELS[budget];
    const isPersonalized = userMetrics && userMetrics.targetCalories > 0;

    useEffect(() => { setEatenItems(getEatenState(formatDateKey(selectedDate))); }, [selectedDate]);
    useEffect(() => { setBudget(getBudget()); }, []);
    useEffect(() => {
        if (calendarRef.current) {
            const todayIdx = dates.findIndex(d => isToday(d));
            calendarRef.current.scrollLeft = Math.max(0, todayIdx - 3) * 62;
        }
    ...
    const toggleEaten = (mealKey: string, idx: number) => {
        const key = `${mealKey}_${idx}`;
    ...
    const totalPlanCal = allItems.reduce((s, i) => s + getItemNutrition(i).cal, 0);
    const totalPlanProtein = allItems.reduce((s, i) => s + getItemNutrition(i).protein, 0);
    ...
    const getMealEatenCals = (mealKey: string, items: MealItem[]) => items.reduce((s, item, i) => s + (eatenItems[`${mealKey}_${i}`] ? getItemNutrition(item).cal : 0), 0);
    const getMealEatenProtein = (mealKey: string, items: MealItem[]) => items.reduce((s, item, i) => s + (eatenItems[`${mealKey}_${i}`] ? getItemNutrition(item).protein : 0), 0);
        ...
        return (
            <div key={mealKey} style={{ background: '#fff', border: `1.5px solid ${allEaten ? '#10b981' : '#e2e8f0'}`, borderRadius: 14, padding: '1rem 1.15rem', marginBottom: '0.75rem', boxShadow: allEaten ? '0 4px 14px rgba(16,185,129,0.08)' : '0 2px 8px rgba(37,99,235,0.03)', transition: 'all 0.3s' }}>
                ...
                {items.map((item, i) => {
                    ...
                    const hasOverride = !!nutritionOverrides[item.name];
                    const isEditing = editingItem === `${mealKey}_${i}`;
                    const grams = parseFloat(item.qty) || 100;
                    return (
                        <div key={i} style={{ marginBottom: 4 }}>
                            <div onClick={() => !isEditing && toggleEaten(mealKey, i)}
                                style={{ display: 'flex', gap: 10, alignItems: 'center', padding: '0.6rem 0.75rem', borderRadius: isEditing ? '10px 10px 0 0' : 10, cursor: isEditing ? 'default' : 'pointer', transition: 'all 0.2s', background: eaten ? 'rgba(16,185,129,0.04)' : '#f8faff', border: `1px solid ${eaten ? 'rgba(16,185,129,0.2)' : '#e2e8f0'}`, borderBottom: isEditing ? '1px dashed #cbd5e1' : undefined }}>
                                ...
                                <div style={{ flex: 1 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                        <span style={{ fontWeight: 600, fontSize: '0.875rem', color: eaten ? '#10b981' : '#0f172a', textDecoration: eaten ? 'line-through' : 'none', transition: 'all 0.2s' }}>{item.name}</span>
                                        {hasOverride && <span style={{ fontSize: '0.58rem', background: '#eff6ff', color: '#2563eb', padding: '1px 5px', borderRadius: 4, fontWeight: 600 }}>Custom</span>}
                                    </div>
                                ...
                                <div style={{ textAlign: 'right', flexShrink: 0, display: 'flex', alignItems: 'center', gap: 6 }}>
                                    <div>
                                        <div style={{ fontSize: '0.8rem', fontWeight: 700, color: eaten ? '#10b981' : '#2563eb' }}>{nutrition.cal} kcal</div>
                                        <div style={{ fontSize: '0.68rem', color: hasOverride ? '#2563eb' : '#94a3b8' }}>P: {nutrition.protein}g</div>
                                    </div>
                                    <button
                                        onClick={(e) => { e.stopPropagation(); if (isEditing) { setEditingItem(null); } else { const o = nutritionOverrides[item.name]; setEditProtein(o ? String(o.protein) : String(Math.round(item.protein * 100 / grams))); setEditCal(o ? String(o.cal) : String(Math.round(item.cal * 100 / grams))); setEditingItem(`${mealKey}_${i}`); } }}
                                        style={{ width: 24, height: 24, borderRadius: 6, border: '1px solid #e2e8f0', background: isEditing ? '#eff6ff' : '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.2s' }}
                                        title="Edit nutrition per 100g"
                                    >
                                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={isEditing ? '#2563eb' : '#94a3b8'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 3a2.85 2.85 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z" /></svg>
                                    </button>
                                </div>
                            </div>
                            {isEditing && (
                                <div onClick={(e) => e.stopPropagation()} style={{ background: '#f8faff', border: '1px solid #e2e8f0', borderTop: 'none', borderRadius: '0 0 10px 10px', padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                                    <div style={{ fontSize: '0.7rem', color: '#64748b', fontWeight: 600 }}>Edit nutrition per 100g for <strong>{item.name}</strong></div>
                                    <div style={{ display: 'flex', gap: 8 }}>
                                        <div style={{ flex: 1 }}>
                                            <label style={{ fontSize: '0.62rem', color: '#94a3b8', fontWeight: 600 }}>Protein / 100g</label>
                                            <input type="number" value={editProtein} onChange={e => setEditProtein(e.target.value)} style={{ width: '100%', padding: '6px 8px', borderRadius: 6, border: '1px solid #e2e8f0', fontSize: '0.8rem', outline: 'none', background: '#fff', marginTop: 2 }} />
                                        </div>
                                        <div style={{ flex: 1 }}>
                                            <label style={{ fontSize: '0.62rem', color: '#94a3b8', fontWeight: 600 }}>Calories / 100g</label>
                                            <input type="number" value={editCal} onChange={e => setEditCal(e.target.value)} style={{ width: '100%', padding: '6px 8px', borderRadius: 6, border: '1px solid #e2e8f0', fontSize: '0.8rem', outline: 'none', background: '#fff', marginTop: 2 }} />
                                        </div>
                                    </div>
                                    <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
                                        {hasOverride && <button onClick={() => resetOverride(item.name)} style={{ padding: '5px 10px', borderRadius: 6, border: '1px solid #fecaca', background: '#fff', color: '#ef4444', fontSize: '0.7rem', fontWeight: 600, cursor: 'pointer' }}>Reset Default</button>}
                                        <button onClick={() => setEditingItem(null)} style={{ padding: '5px 10px', borderRadius: 6, border: '1px solid #e2e8f0', background: '#fff', color: '#64748b', fontSize: '0.7rem', fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
                                        <button onClick={() => { const p = parseFloat(editProtein); const c = parseFloat(editCal); if (p >= 0 && c >= 0) saveOverride(item.name, p, c); }} style={{ padding: '5px 12px', borderRadius: 6, border: 'none', background: 'linear-gradient(135deg, #2563eb, #06b6d4)', color: '#fff', fontSize: '0.7rem', fontWeight: 700, cursor: 'pointer' }}>Save</button>
                                    </div>
                                </div>
                            )}
                        </div>
        ...
        <div style={{ minHeight: '100vh', background: '#f8faff', paddingTop: 70 }}>
            ...
            <div style={{ maxWidth: 1000, margin: '0 auto', padding: '2rem 1.5rem', animation: 'fadeIn 0.5s ease' }}>
                ...
                <div style={{ display: 'flex', gap: 8, marginBottom: '1.25rem' }}>
                    {[
                        { key: 'tracker', label: '📅 Meal Tracker' },
                        { key: 'log', label: '📋 Food Log' },
                    ].map(t => (
                        <button key={t.key} onClick={() => setActiveTab(t.key as any)}
                            style={{ flex: 1, padding: '0.7rem 1rem', borderRadius: 12, border: activeTab === t.key ? 'none' : '1px solid #e2e8f0', cursor: 'pointer', transition: 'all 0.2s', fontWeight: 700, fontSize: '0.9rem', background: activeTab === t.key ? 'linear-gradient(135deg, #2563eb, #06b6d4)' : '#fff', color: activeTab === t.key ? '#fff' : '#64748b', boxShadow: activeTab === t.key ? '0 4px 14px rgba(37,99,235,0.25)' : '0 1px 4px rgba(0,0,0,0.02)' }}>
                            {t.label}
                        </button>
                    ))}
                </div>
                    ...
                    <div>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                                <span style={{ background: `${budgetInfo.color}15`, color: budgetInfo.color, padding: '4px 12px', borderRadius: 8, fontSize: '0.78rem', fontWeight: 700, border: `1px solid ${budgetInfo.color}30` }}>
                                    {budgetInfo.emoji} {budgetInfo.label}
                                </span>
                                {isPersonalized && (
                                    <span style={{ background: 'rgba(37,99,235,0.06)', color: '#2563eb', padding: '4px 12px', borderRadius: 8, fontSize: '0.72rem', fontWeight: 700, border: '1px solid rgba(37,99,235,0.15)' }}>
                                        ⚡ Personalized · {userMetrics!.targetCalories} kcal · {userMetrics!.weight}kg
                                    </span>
                                )}
                            </div>
                            <Link href="/profile" style={{ color: '#2563eb', fontSize: '0.75rem', fontWeight: 600, textDecoration: 'none' }}>Change Budget →</Link>
                        </div>

                        {/* Day Label */}
                        <div style={{ color: '#94a3b8', fontSize: '0.72rem', fontWeight: 600, marginBottom: '0.75rem' }}>
                            📋 {dayFullNames[selectedDate.getDay()]}&apos;s Plan (Day {(selectedDate.getDay() === 0 ? 7 : selectedDate.getDay())} of 7)
                            {isPersonalized && <span> · Scaled to your body</span>}
                        </div>

                        {/* Horizontal Calendar */}
                        <div ref={calendarRef} style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 8, marginBottom: '0.75rem', scrollBehavior: 'smooth', WebkitOverflowScrolling: 'touch' }} className="hide-scrollbar">
                            {dates.map((d, i) => {
                        ...
                        <div style={{ position: 'sticky', top: 64, zIndex: 30, background: '#f8faff', paddingTop: 8, paddingBottom: 4 }}>
                            <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 14, padding: '0.85rem 1.25rem', marginBottom: '0.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.75rem', boxShadow: '0 2px 12px rgba(37,99,235,0.06)' }}>
                                ...
                                <div style={{ display: 'flex', gap: 10 }}>
                                    ...
                                    <div style={{ textAlign: 'center' }}>
                                        <div style={{ fontSize: '1.3rem', fontWeight: 900, color: '#2563eb' }}>{totalEatenCal}</div>
                                        <div style={{ fontSize: '0.65rem', color: '#94a3b8' }}>kcal eaten</div>
                                    </div>
                                    <div style={{ textAlign: 'center' }}>
                                        <div style={{ fontSize: '1.3rem', fontWeight: 900, color: '#06b6d4' }}>{totalEatenProtein}g</div>
                        ...
                        <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 14, padding: '1.25rem', boxShadow: '0 2px 12px rgba(37,99,235,0.04)' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.5rem' }}>
                                {[
                                    { label: 'Total Cal', value: `${totalPlanCal}`, sub: `Eaten: ${totalEatenCal}`, color: '#2563eb' },
                                    { label: 'Total Protein', value: `${totalPlanProtein}g`, sub: `Eaten: ${totalEatenProtein}g`, color: '#06b6d4' },
                                    { label: 'Items', value: `${eatenCount}/${totalItems}`, sub: `${adherence}%`, color: '#10b981' },
                                    { label: 'Remaining', value: `${totalPlanCal - totalEatenCal}`, sub: 'kcal left', color: '#f59e0b' },
                                ].map(c => (
                                ...
                                <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 14, padding: '1.25rem', boxShadow: '0 2px 12px rgba(37,99,235,0.04)' }}>
                                    ...
                                    {selectedFood && (
                                        <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 10, padding: '0.9rem', marginBottom: '0.9rem' }}>
                                            ...
                                            {(() => { const c = calc(selectedFood, Number(grams)); return (<div style={{ background: '#fff', borderRadius: 8, padding: '0.6rem 0.75rem', fontSize: '0.8rem', color: '#64748b' }}>For {grams}g → <strong style={{ color: '#2563eb' }}>{c.cal} kcal</strong> · P:{c.pro}g · C:{c.carb}g · F:{c.fat}g</div>); })()}
                                        </div>
                                    )}
                                ...
                                {MEALS.map(m => {
                                    ...
                                    return (
                                        <div key={m} style={{ marginBottom: '1rem' }}>
                                            ...
                                            {entries.length === 0 ? (<div style={{ color: '#cbd5e1', fontSize: '0.8rem', padding: '0.5rem 0' }}>Nothing logged</div>) : entries.map(e => (
